...
def gauss_smoothen_image(cfg, img, sigma_rel):
    ...
    img_tmp = img
    img_tmp = tf.nn.depthwise_conv2d(input=img_tmp, filter=k1, strides=[1, 1, 1, 1], padding="SAME")
    img_tmp = tf.nn.depthwise_conv2d(input=img_tmp, filter=k2, strides=[1, 1, 1, 1], padding="SAME")
    return img_tmp
