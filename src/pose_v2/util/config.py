...
def _merge_a_into_b(a, b, type_conversion=False):
    ...
    if not (type(a) is edict or type(a) is dict):
        raise TypeError("parameter a must be of dict type, got", type(a)," instead.")
    ...
    for k, v in a.items():
        ...
        if type_conversion:
            ...
            if type(v) == bool and param_type != bool:
                raise TypeError(
                    "The type of the parameter", k, "is", param_type.__name__ ,", but a value", v ," is supplied.")
            v = param_type(v)
        else:
            # the parameter types must match those in the default config file
            if type(v) != param_type:
                raise TypeError("The type of the parameter", k ,"is",  param_type.__name__ , ", but a value ", v ," is supplied.")
