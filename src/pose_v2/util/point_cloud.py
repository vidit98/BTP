...
def pointcloud2voxels(cfg, input_pc, sigma):  # [B,N,3]
    ...
    if cfg.pc_normalise_gauss:
        normaliser = tf.reduce_sum(input_tensor=func, axis=[2, 3, 4], keepdims=True)
        func /= normaliser
    ...
    summed = tf.reduce_sum(input_tensor=func, axis=1)  # [B,G,G G]
...
def pointcloud2voxels3d_fast(cfg, pc, rgb):  # [B,N,3]
    ...
    valid = tf.reduce_all(input_tensor=valid, axis=-1)
    ...
    if filter_outliers:
        ...
        indices = tf.boolean_mask(tensor=indices, mask=valid)

    def interpolate_scatter3d(pos):
        updates_raw = rr[pos[0]][:, :, 0] * rr[pos[1]][:, :, 1] * rr[pos[2]][:, :, 2]
        updates = tf.reshape(updates_raw, [-1])
        if filter_outliers:
            updates = tf.boolean_mask(tensor=updates, mask=valid)

        indices_loc = indices
        indices_shift = tf.constant([[0] + pos])
        num_updates = tf.shape(input=indices_loc)[0]
        ...
        if has_rgb:
            ...
            if filter_outliers:
                updates_rgb = tf.boolean_mask(tensor=updates_rgb, mask=valid)
...
def pc_perspective_transform(cfg, point_cloud,
    ...
    if cfg.pose_quaternion:
        ...
        xyz1 = tf.pad(tensor=point_cloud, paddings=tf.constant([[0, 0], [0, 0], [0, 1]]), mode="CONSTANT", constant_values=1.0)

        extrinsic = transform
        intr = intrinsic_matrix(cfg, dims=4)
        intrinsic = tf.convert_to_tensor(value=intr)
        intrinsic = tf.expand_dims(intrinsic, axis=0)
        intrinsic = tf.tile(intrinsic, [tf.shape(input=extrinsic)[0], 1, 1])
        full_cam_matrix = tf.matmul(intrinsic, extrinsic)

        pc2 = tf.matmul(xyz1, tf.transpose(a=full_cam_matrix, perm=[0, 2, 1]))
...
def pointcloud_voxelize(cfg, point_cloud, transform, sigma):
   
    tr_pc = pc_perspective_transform(cfg, point_cloud, transform)
    voxels = pointcloud2voxels(cfg, tr_pc, sigma)
    voxels = tf.transpose(a=voxels, perm=[0, 2, 1, 3, 4])
...
def pc_point_dropout(points, rgb, keep_prob):
    ...
    selected_indices = tf.compat.v1.py_func(sampler, [num_output_points], tf.int64)
