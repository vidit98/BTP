...
def vector3d_to_quaternion(x):
    ...
    return tf.pad(tensor=x, paddings=(len(x.shape) - 1) * [[0, 0]] + [[1, 0]])
...
def quaternion_normalise(q):
    ...
    Returns:
        q / ||q||_2
    """
    return q / tf.norm(tensor=q, axis=-1, keepdims=True)
...
def quaternion_rotate(pc, q, inverse=False):
    ...
    q_norm = tf.expand_dims(tf.norm(tensor=q, axis=-1), axis=-1)
    q /= q_norm
...
def from_rotation_matrix(mtr):
    ...
    """
    mtr = tf.convert_to_tensor(value=mtr)
    def m(j, i):
