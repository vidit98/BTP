...
def regularization_loss(scopes, cfg, postfix=""):
    ...
    if cfg.weight_decay > 0:
        ...
        for scope in scopes:
            scope_vars = [x for x in tf.compat.v1.trainable_variables() if x.name.startswith(scope)]
            scope_vars_w = list(filter(is_weights, scope_vars))
            if scope_vars_w:
                reg_loss += tf.add_n([tf.nn.l2_loss(var) for var in scope_vars_w])

    tf.compat.v2.summary.scalar(name="losses/reg_loss" + postfix, data=reg_loss, step=tf.compat.v1.train.get_or_create_global_step())
...
def drc_loss(cfg, probs, gt_proj):
    ...
    return tf.reduce_sum(input_tensor=probs * psi) #/ tf.to_float(vox_size)
...
def drc_rgb_loss(cfg, probs, rgb, gt):
    ...
    probs = tf.transpose(a=probs, perm=[1, 0, 2, 3, 4])

    psi = tf.square(gt_vol - rgb_pred)
    psi = tf.reduce_sum(input_tensor=psi, axis=4, keepdims=True)
...
def add_drc_loss(cfg, inputs, outputs, weight_scale, add_summary=True):
    ...
    if gt_size != pred_size:
        gt = tf.image.resize(gt, [pred_size, pred_size])

    loss = drc_loss(cfg, pred, gt)
    loss /= tf.cast(num_samples, dtype=tf.float32)
    if add_summary:
        tf.compat.v2.summary.scalar(name="losses/drc_loss", data=loss, step=tf.compat.v1.train.get_or_create_global_step())
    loss *= weight_scale
    return loss
